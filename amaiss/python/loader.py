# AMAISS loader - runtime CPU feature detection and module loading
# Adapted from FAISS (Meta Platforms, Inc.)
...
opt_env_variable_name = "AMAISS_OPT_LEVEL"
...
if has_AVX512 and not loaded:
    try:
        logger.info("Loading amaiss with AVX512 support.")
        from .swigamaiss_avx512 import *

        logger.info("Successfully loaded amaiss with AVX512 support.")
        loaded = True
    except ImportError as e:
...
if has_AVX2 and not loaded:
    try:
        logger.info("Loading amaiss with AVX2 support.")
        from .swigamaiss_avx2 import *

        logger.info("Successfully loaded amaiss with AVX2 support.")
        loaded = True
    except ImportError as e:
...
if has_SVE and not loaded:
    try:
        logger.info("Loading amaiss with SVE support.")
        from .swigamaiss_sve import *

        logger.info("Successfully loaded amaiss with SVE support.")
        loaded = True
    except ImportError as e:
...
if not loaded:
    try:
        logger.info("Loading amaiss (generic).")
        from .swigamaiss import *

        logger.info("Successfully loaded amaiss.")
