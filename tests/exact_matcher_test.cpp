...
amaiss::SparseVectors make_u8_vectors(
    size_t dimension,
    const std::vector<std::vector<std::pair<amaiss::term_t, uint8_t>>>& vecs) {
    amaiss::SparseVectors sv(amaiss::SparseVectorsConfig{
...
TEST(ExactMatcher, single_query_returns_top_k_by_score) {
    ...
    std::vector<amaiss::idx_t> ids = {0, 1, 2};
    amaiss::ArrayIDSelector selector(ids.size(), ids.data());

    auto [scores, labels] = amaiss::detail::ExactMatcher::single_query(
        &vectors, &selector, dense.data(), amaiss::U8, 2);
...
TEST(ExactMatcher, single_query_pads_to_k_when_fewer_docs) {
    ...
    std::vector<amaiss::idx_t> ids = {0};
    amaiss::ArrayIDSelector selector(ids.size(), ids.data());

    auto [scores, labels] = amaiss::detail::ExactMatcher::single_query(
        &vectors, &selector, dense.data(), amaiss::U8, 3);
    ...
    // padded entries
    EXPECT_EQ(labels[1], amaiss::INVALID_IDX);
    EXPECT_FLOAT_EQ(scores[1], -1.0F);
    EXPECT_EQ(labels[2], amaiss::INVALID_IDX);
...
TEST(ExactMatcher, single_query_subset_of_ids) {
    ...
    std::vector<amaiss::idx_t> ids = {0, 2};
    amaiss::ArrayIDSelector selector(ids.size(), ids.data());
...
TEST(ExactMatcher, single_query_zero_scores) {
    ...
    std::vector<amaiss::idx_t> ids = {0, 1};
    amaiss::ArrayIDSelector selector(ids.size(), ids.data());
...
TEST(ExactMatcher, search_multiple_queries) {
    ...
    auto [result_distances, result_labels] =
        amaiss::detail::ExactMatcher::search(&vectors, &selector, &queries,
                                             amaiss::U8, 2);
...
TEST(ExactMatcher, search_with_k_greater_than_docs) {
    ...
    EXPECT_EQ(result_labels[0][1], amaiss::INVALID_IDX);
    EXPECT_EQ(result_labels[0][2], amaiss::INVALID_IDX);
}
...
TEST(ExactMatcher, single_query_with_set_id_selector) {
    ...
    std::vector<amaiss::idx_t> ids = {1, 2};
    amaiss::SetIDSelector selector(ids.size(), ids.data());
